...
function register ($data) {
	...
	$pjgpass=strlen($password);
	$pjgnip=strlen($nip);
	// cek apakah nip angka semua bukan?
	if (!filter_var($nip, FILTER_VALIDATE_INT) === TRUE) {
	// kalo bukan
		echo "<script>
		alert('NIP bukan Angka! Gagal Melakukan Pedanftaran!')
		</script>"; 
	//   kalau iya angka
	}else{
		// cek panjang nip 18digit ?
	if ($pjgnip < 18 OR $pjgnip > 18) {
		echo "<script>
		alert('Cek Kembali NIP, NIP tidak bisa Lebih Dari atau Kurang Dari 18')
		</script>"; 
	}else{
	// cek apakah nip sudah ada di database alternatif / sudah pernah daftar ?
	$ceknip = mysqli_query($koneksi, "SELECT nip FROM alternatif WHERE nip='$nip'");
	if (mysqli_num_rows($ceknip)>0) {
		echo "<script>
			alert('NIP Sudah Terdaftar, Gagal Mendaftar Akun!');
			</script>";
	}else{
	...
	mysqli_query($koneksi,"INSERT INTO alternatif (id_periode, kode, nip, nama, username,password) VALUES('$data[id_periode]','$kode','$nip','$nama','$username','$password')");
	return mysqli_affected_rows($koneksi);
	}
}
}
...
function nambah($nama,$batas,$id_p) {
	...
	foreach ($nama as $kunci => $val) {
		...
		if (mysqli_num_rows($query_cek)>0) {
			...
			if ($i<=$batas) {
				$insert=mysqli_query($koneksi, "INSERT INTO hasil (id_alternatif, id_periode,nilai,tanggal,ket) VALUES ('$kunci','$id_p','$val','$tanggal','baik')");
			}else{
				$insert=mysqli_query($koneksi, "INSERT INTO hasil (id_alternatif, id_periode,nilai,tanggal,ket) VALUES ('$kunci','$id_p','$val','$tanggal','kurang baik')");
			}	
		$i++;
	}
}
