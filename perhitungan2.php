				...
				<table class="table table-bordered">
					<thead class="table-dark" style="background-color: #548176">
						<tr>
							...
							<th class="text-center" style="vertical-align: middle;" rowspan="2">Nama ALT</th>
							<th class="text-center" style="vertical-align: middle;" rowspan="2">Kode ALT</th>
							<?php foreach ($kriteria as $key) : ?>
								<?php if ($key['id'] == 1) : ?>
									<th class="text-center" style="vertical-align: middle;" rowspan="2"><?= $key['nama_kriteria'] ?></th>
								<?php else : ?>
									<?php $idkck = $key['id_kriteria'];
									$cekrw = query("SELECT*FROM subkriteria where id_kriteria=$idkck"); ?>
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					<tbody>
						...
						$alternatif = query("SELECT * FROM alternatif WHERE status='1' AND id_periode='$id_p'");
						$kriteria = query("SELECT *FROM kriteria");
						$nilai_itung = query("SELECT alternatif.*,nilai_alt.*,kriteria.*
						...
						<?php foreach ($alternatif as $key2) : ?>
							<tr>
								...
								<?php $idal = $key2['id_alternatif'];
								foreach ($kriteria as $key3) :
									$idk = $key3['id_kriteria'];
									$ceksub = query("SELECT*FROM subkriteria where id_kriteria=$idk");
									if (count($ceksub) > 0) :
										$ns = query("SELECT id_subkriteria FROM subkriteria where id_kriteria=$idk");
										foreach ($ns as $key4) :
											$idsubb = $key4['id_subkriteria'];
								...
								?>
									...
									<?php else :
										$pecah = str_split($nilai_normal[$idal][$idk], 8); ?>
										<td class="text-center"><?= $pecah[0]; ?></td>
									<?php endif; ?>
